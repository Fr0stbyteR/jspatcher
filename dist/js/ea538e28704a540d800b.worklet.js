...
const _Project = class extends _utils_TypedEventEmitter__WEBPACK_IMPORTED_MODULE_0__["default"] {
  constructor(envIn, props) {
    ...
    this.props = {
      ...
      systemVersion: _Project.props.systemVersion.default
    };
    this.ready = false;
    this.env = envIn;
    if (props)
  ...
  async init() {
    if (await this.env.fileMgr.exists(`/project/${this.projectFilename}`)) {
      const item = this.env.fileMgr.getProjectItemFromPath(`./${this.projectFilename}`);
      const props = JSON.parse((0,_utils_utils__WEBPACK_IMPORTED_MODULE_1__.ab2str)(item.data));
      for (const keyIn in props) {
        const key = keyIn;
        if (this.props[key] === props[key])
          continue;
...
Project.props = {
  ...
  systemVersion: {
    ...
    default: _version__WEBPACK_IMPORTED_MODULE_2__["default"]
  }
};
...
class WorkletGlobalPackageManager {
  constructor(envIn) {
    this.externals = /* @__PURE__ */ new Map();
...
class FileEditor extends _utils_TypedEventEmitter__WEBPACK_IMPORTED_MODULE_0__["default"] {
  constructor(instance) {
    ...
    this.on("dirty", async (isDirty) => {
      var _a2;
      if (isDirty && this.env.autoSave && !this.isReadonly && !this.isInMemory)
        await this.save();
      else
        (_a2 = this.file) == null ? void 0 : _a2.emit("dirty", isDirty);
    });
    this.env.on("options", ({ options: { autoSave } }) => {
      if (autoSave && !this.isReadonly && !this.isInMemory)
        this.save();
    });
    this.on("destroy", () => {
      var _a2;
...
class FileInstance extends _utils_TypedEventEmitter__WEBPACK_IMPORTED_MODULE_0__["default"] {
  constructor({ env, project, file, instanceId, noRegister }) {
    super();
...
class History extends _utils_TypedEventEmitter__WEBPACK_IMPORTED_MODULE_0__["default"] {
  constructor() {
    super(...arguments);
    ...
    this.handleEditorEvent = async (eventName, eventData, editor) => {
      ...
      const event = {
        ...
        fileId: (_a = editor.file) == null ? void 0 : _a.id,
  ...
  get now() {
    return (0,_utils_utils__WEBPACK_IMPORTED_MODULE_1__.getTimestamp)();
  }
  get firstEditor() {
  ...
  get fileId() {
    var _a;
    return (_a = this.firstEditor.file) == null ? void 0 : _a.id;
  }
  addEditor(editor) {
  ...
  async undo(passive = false, ...editors) {
    if (!this.isUndoable)
      return;
    this.capture = false;
    ...
    this.capture = true;
    if (!passive)
      this.emitChanged({ prevHistoryIndex: this.$ + 1, nextHistoryIndex: this.$, timestamp: this.now, fileId: this.fileId });
  }
  async redo(passive = false, ...editors) {
    if (!this.isRedoable)
      return;
    this.capture = false;
  ...
  async undoUntil($, passive = false, ...editors) {
    while (this.isUndoable && $ < this.$) {
      await this.undo(passive, ...editors);
    }
  }
  async redoUntil($, passive = false, ...editors) {
    while (this.isRedoable && $ > this.$) {
      await this.redo(passive, ...editors);
    }
  }
  async setIndex($, passive = false) {
    if ($ < this.$)
      await this.undoUntil($, passive);
    else if ($ > this.$)
      await this.redoUntil($, passive);
  }
  getSyncData() {
  }
  async syncData(data) {
  ...
  async unmergeChanges(...changes) {
    ...
    while ($change >= 0) {
      ...
      if (eventData) {
        ...
        const i2 = this.eventQueue.findIndex((e) => e.timestamp === timestamp);
        if (i2 >= 0)
          this.eventQueue.splice(i2);
      } else {
...
class PersistentProjectFile extends _AbstractProjectFile__WEBPACK_IMPORTED_MODULE_0__["default"] {
  ...
  async instantiate({ env, project, instanceId }) {
    ...
    const Constructor = {
      ...
      video: (await __webpack_require__.e(/*! import() */ "src_core_video_PatcherVideo_ts").then(__webpack_require__.bind(__webpack_require__, /*! ../video/PatcherVideo */ "./src/core/video/PatcherVideo.ts"))).default,
  ...
  async instantiateEditor({ env, project, instanceId }) {
    ...
    const Constructor = {
      ...
      video: (await __webpack_require__.e(/*! import() */ "src_core_video_VideoEditor_ts").then(__webpack_require__.bind(__webpack_require__, /*! ../video/VideoEditor */ "./src/core/video/VideoEditor.ts"))).default,
...
class PersistentProjectFolder extends _AbstractProjectFolder__WEBPACK_IMPORTED_MODULE_1__["default"] {
  ...
  async destroy() {
    var _a;
    await this._fileMgr.remove(this.path, this.isFolder);
    (_a = this.parent) == null ? void 0 : _a.items.delete(this);
    await this.emitTreeChanged();
    await this.emit("destroyed");
  }
}
...
class Setter extends _ImportedObject__WEBPACK_IMPORTED_MODULE_0__["default"] {
  constructor() {
    ...
    this._ = { instance: void 0, input: void 0 };
    this.handleInlet = ({ data, inlet }) => {
      if (inlet === 0) {
        if (!(0,_base_Bang__WEBPACK_IMPORTED_MODULE_1__.isBang)(data))
          this._.instance = data;
...
class SetterGetter extends _ImportedObject__WEBPACK_IMPORTED_MODULE_0__["default"] {
  constructor() {
    ...
    this.initialInlets = 2;
    this.initialOutlets = 2;
    this._ = { instance: void 0, input: new _base_Bang__WEBPACK_IMPORTED_MODULE_1__["default"](), result: null };
    this.handleInlet = ({ data, inlet }) => {
      if (inlet === 0) {
        if (!(0,_base_Bang__WEBPACK_IMPORTED_MODULE_1__.isBang)(data))
          this._.instance = data;
        if (typeof this._.instance === "undefined")
          return;
        if (!(0,_base_Bang__WEBPACK_IMPORTED_MODULE_1__.isBang)(this._.input)) {
          try {
            this._.instance[this.name] = this._.input;
          } catch (e) {
            this.error(e);
...
class In extends _base_BaseObject__WEBPACK_IMPORTED_MODULE_0__["default"] {
  ...
  subscribe() {
    ...
    if (this.env.thread === this.thread)
...
class Box extends _utils_TypedEventEmitter__WEBPACK_IMPORTED_MODULE_0__["default"] {
  constructor(patcherIn, boxIn) {
    ...
    this._inletLines = new Array(this.inlets).fill(null).map(() => /* @__PURE__ */ new Set());
    this._outletLines = new Array(this.outlets).fill(null).map(() => /* @__PURE__ */ new Set());
  ...
  addInletLine(line) {
    ...
    if (!this._inletLines[index])
      this._inletLines[index] = /* @__PURE__ */ new Set();
  ...
  addOutletLine(line) {
    ...
    if (!this._outletLines[index])
      this._outletLines[index] = /* @__PURE__ */ new Set();
  ...
  setInlets(count) {
    ...
    if (count > linesSetLength)
      this._inletLines.push(...new Array(count - linesSetLength).fill(null).map(() => /* @__PURE__ */ new Set()));
  ...
  setOutlets(count) {
    ...
    if (count > linesSetLength)
      this._outletLines.push(...new Array(count - linesSetLength).fill(null).map(() => /* @__PURE__ */ new Set()));
    else if (count < linesSetLength)
...
class Line extends _utils_TypedEventEmitter__WEBPACK_IMPORTED_MODULE_0__["default"] {
  ...
  static isConnectableByAudio(from, outlet, to, inlet) {
    const fromConnection = from == null ? void 0 : from.outletAudioConnections[outlet];
    const toConnection = to == null ? void 0 : to.inletAudioConnections[inlet];
    if (!fromConnection)
      return false;
    if (!toConnection)
      return false;
      ...
      if (from && to && from.node && to.node) {
        ...
        try {
          ...
          if (Line.isWamNode(from.node) && Line.isWamNode(to.node))
            from.node.disconnectEvents(to.node.instanceId);
        } catch (e) {
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ("0.0.10.1655288722740");
...
class WorkletEnvProcessor extends _AudioWorkletProxyProcessor__WEBPACK_IMPORTED_MODULE_0__["default"] {
  ...
  generateId(objectIn) {
    var _a, _b;
    return this.thread + objectIn.constructor.name + ((_b = (_a = globalThis.Atomics) == null ? void 0 : _a.add(this.generatedId, 0, 1)) != null ? _b : ++this.generatedId[0]);
  }
...
/******/ })()
;
//# sourceMappingURL=ea538e28704a540d800b.worklet.js.map