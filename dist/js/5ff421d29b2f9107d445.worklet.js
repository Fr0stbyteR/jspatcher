...
class FileEditor extends _utils_TypedEventEmitter__WEBPACK_IMPORTED_MODULE_0__["default"] {
  constructor(instance) {
    ...
    this.on("dirty", async (isDirty) => {
      var _a2;
      if (this.env.autoSave && !this.isReadonly && !this.isInMemory)
        await this.save();
      else
...
class History extends _utils_TypedEventEmitter__WEBPACK_IMPORTED_MODULE_0__["default"] {
  ...
  emitChanged(event) {
    ...
    if (event)
      ...
      this.emit("change", { currentHistoryIndex: this.$, timestamp: this.now, editorId: this.firstEditor.editorId, fileId: (_a = this.firstEditor.file) == null ? void 0 : _a.id });
  ...
  async setIndex($, passive = false) {
    if ($ < this.$)
      await this.undoUntil(this.eventQueue[$].timestamp, passive);
    else if ($ > this.$)
      await this.redoUntil(this.eventQueue[$ - 1].timestamp, passive);
  }
  getSyncData() {
...
/******/ })()
;
//# sourceMappingURL=5ff421d29b2f9107d445.worklet.js.map