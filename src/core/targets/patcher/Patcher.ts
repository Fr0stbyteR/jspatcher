...
import type { TLine, TBox, PatcherMode, RawPatcher, TErrorLevel, TFlatPackage, TPackage, ILogInfo, TDependencies } from "../types";
...
export default class Patcher extends FileInstance<PatcherEventMap, PersistentProjectFile | TempPatcherFile> {
    ...
    async load(patcherIn: RawPatcher | any, modeIn?: PatcherMode) {
        ...
        //     let patcher;
        //     if (mode === "js" || mode === "faust" || mode === "jsaw") {
    ...
    canCreateLine(lineIn: TLine) {
        if (lineIn.src[1] >= this.boxes[lineIn.src[0]].ios) return false;
        if (lineIn.dest[1] >= this.boxes[lineIn.dest[0]].ios) return false;
    ...
    getLinesBySrcID(srcId: string) {
        const result = [];
        for (let i = 0; i < this.boxes[srcId].ios; i++) { // Array.fill fills the array with same instance
            result[i] = [];
        }
    ...
    getLinesByDestID(destId: string) {
        const result = [];
        for (let i = 0; i < this.boxes[destId].ios; i++) {
            result[i] = [];
        }
        for (const id in this.lines) {
            const line = this.lines[id];
    ...
    inspectAudioIO() {
        ...
        for (const boxId in this.boxes) {
            ...
            if (box.meta.isPatcherInlet === "parameter")
    ...
    get metaFromPatcher(): Pick<IJSPatcherObjectMeta, "ios" | "args" | "props"> {
        const ios: IJSPatcherObjectMeta["ios"] = [];
        for (const boxId in this.boxes) {
            ...
            // TODO -- this needs to be fixed for the I/O semantics
            if (box.meta.isPatcherInlet === "data" && !ios[port]) {
                ios[port] = {
                    type: box.props.type || "anything",
                    description
                };
            } else if (box.meta.isPatcherOutlet === "data" && !ios[port]) {
                ios[port] = {
                    type: box.props.type || "anything",
                    description
                };
