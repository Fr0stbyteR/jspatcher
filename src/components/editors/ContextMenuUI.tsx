...
export default class ContextMenuUI extends React.PureComponent<P, S> {
    ...
    handleOpenHelp = () => {
        this.setState({ open: false });
        if (this.props.editor instanceof PatcherEditor)
            this.props.editor.emit('help');
        else
            this.props.editor.emit('help');
    }
    handleOpenReference = () => {
        this.setState({ open: false });
        if (this.props.editor instanceof PatcherEditor)
            this.props.editor.emit('reference');
        else
            this.props.editor.emit('reference');
    }
    handleBringToFront = () => {
    ...
    handleInspector = () => {
        this.setState({ open: false });
        if (this.props.editor instanceof PatcherEditor)
            this.props.editor.emit('inspector');
        else
            this.props.editor.emit('inspector');
    }
    render() {
                    ...
                    >
                        ...
                        {this.props.editor.isLocked ? undefined : <>
                            {this.state.singleSelection ? <>
                                ...
                                <Menu.Item key='reference' content={`Open ${this.state.singleSelection} Reference`} icon='book' onClick={this.handleOpenReference} disabled={!Boolean(this.state.reference)} />
